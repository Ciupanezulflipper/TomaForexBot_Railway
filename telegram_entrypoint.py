...
async def chart_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 2:
        ...
        return
    symbol, tf = context.args[0].upper(), context.args[1].upper()
    try:
...
async def calendar_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not events:
            ...
            return
        msg = "\n".join(f"{e['date']} - {e['event']} - {e['impact']}" for e in events[:6])
        await update.message.reply_text(msg)
    except Exception as e:
...
def start_telegram_listener():
    import asyncio
    asyncio.run(start_telegram_listener_async())
...
async def start_telegram_listener_async():
    ...
    logger.info("🤖 Telegram bot started (asyncio mode)...")
